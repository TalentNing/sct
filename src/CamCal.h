...
//! define the initial population of EDA (default: 2000)
#define EDA_INIT_POP (2000)
...
public:
	...
	bool process(std::vector<cv::Point> voVanPt, std::vector<C1dNd> vo1dNd, std::vector<int> vn1dNdInstNum, bool bOvrdFlg = false);
...
private:
	//! verifies that the number of candidate points for the estimation Vy and Linf are sufficient
	bool vfyCandNum(std::vector<C1dNd> vo1dNd, std::vector<int> vn1dNdInstNum);
	//! collects candidate points for vertical vanishing point (Vy) estimation
	void colVyCand(std::vector<C1dNd> vo1dNd);
	//! collects candidate points for horizon line (Linf) estimation
	void colLinfCand(std::vector<C1dNd> vo1dNd, int nMax1dNdId);
	...
	//! calculates reprojection error based on distance between grid points to grid lines
	double calcReprojErr(cv::Point2f oVr, cv::Point2f oVl, cv::Point oStGrdPt, CCamParam* poCamParam);
	...
	//! plots 2D foot/head points
	void plt2dFtHd(std::vector<C1dNd> vo1dNd);
	//! plots vanishing points and horizon line
	void pltVpLinf(void);
...
// selector of pairs of test points for measuring 3D distance
class CTstPtSel
{
public:
	CTstPtSel(void);
	...
	//! initializes the test points selector
	void initialize(CCfg oCfg, cv::Mat oImgBg);
	//! selects test points and measures 3D distance
	void process(void);
