...
public:
	CDetNd(void);
	CDetNd(cv::Rect2f oDetBBox, float fDetScr, cv::Point3f oDet3dFtPt, char* acDetCls, int nFrmCnt = -1);
	~CDetNd(void);
	...
	inline cv::Point3f getDet3dFtPt(void) { return m_oDet3dFtPt; }
	inline void setDet3dFtPt(cv::Point3f oDet3dFtPt) { m_oDet3dFtPt = oDet3dFtPt; }
...
protected:
	...
	float m_fDetScr;
	//! 3D foot point of object detection
	cv::Point3f m_oDet3dFtPt;
...
private:
	...
	////! network of YOLO
	//network* m_poYoloNet;	// in Linux
	////! meta data of YOLO
	//metadata m_oYoloMeta;	// in Linux
	////! C4 scanner
